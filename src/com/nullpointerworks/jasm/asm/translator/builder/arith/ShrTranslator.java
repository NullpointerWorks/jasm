...
public class ShrTranslator implements CodeTranslator
{
	...
	public List<Translation> getTranslation(SourceCode sc)
	{
		...
		{
			error = new TranslationError(sc, "  Syntax error: SHR instruction is incomplete."+syntax);
		}
	...
	private void translate(SourceCode sc, String operands, List<Translation> translation) 
	{
		...
		if (tokens.length != 1) 
		{
			error = new TranslationError(sc, "  Syntax error: SHR instructions take one argument."+syntax);
			return;
		}
	...
	private void allow(SourceCode sc, Operand op1, List<Translation> translation) 
	{
		...
		t.setInstruction(ASMInstruction.SHR);
		t.setOperand(op1);
		translation.add(t);
	}
