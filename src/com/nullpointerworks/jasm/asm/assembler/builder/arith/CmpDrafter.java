...
public class CmpDrafter implements Drafter
{
	...
	{
		return instruct == ASMInstruction.CMP;
	}
							...
							List<Definition> defs)
	{
		...
		if (op1.isRegister())
		{
			if (op2.isRegister())
			{
				setCode(d, VMInstruction.CMP_RR, op1.getRegister(), op2.getRegister());
				return d;
			}
			if (op2.isNumber()) 
			{
				setCode(d, VMInstruction.CMP_RV, op1.getRegister(), op2.getInteger());
				return d;
			}
			if (op2.isLabel()) 
			{
				...
				if (def != null)
				{
					setCode(d, VMInstruction.CMP_RV, op1.getRegister(), def.getNumber() );
					return d;
				}
