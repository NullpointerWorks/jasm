...
public class SubDrafter implements Drafter
{
	...
	{
		return instruct == ASMInstruction.SUB;
	}
							...
							List<Definition> defs)
	{
		...
		if (op1.isRegister())
		{
			if (op2.isRegister())
			{
				setCode(d, VMInstruction.SUB_RR, op1.getRegister(), op2.getRegister());
				return d;
			}
			if (op2.isNumber()) 
			{
				setCode(d, VMInstruction.SUB_RV, op1.getRegister(), op2.getInteger());
				return d;
			}
			if (op2.isLabel()) 
			{
				...
				if (def != null)
				{
					setCode(d, VMInstruction.SUB_RV, op1.getRegister(), def.getNumber() );
					return d;
				}
